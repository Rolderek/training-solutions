...
public class EmployeeService {
    ...
    public long sumSalaries(List<Employee> employees) {
        ...
        /*
        return employees.stream().collect(Collectors.summarizingInt(Employee::getSalary)).getSum();
         */
    }
    ...
    public Map<Boolean, List<Employee>> partitionBySalary(List<Employee> employees, int boundry) {
        return employees.stream()
                .collect(Collectors.partitioningBy(e -> e.getSalary() >= boundry));
    }

    public Map<String, Optional<Integer>> minSalaryByDivision(List<Employee> employees) {
        return employees.stream()
                .collect(Collectors.groupingBy(Employee::getDivision, Collectors.mapping(Employee::getSalary, Collectors.minBy(Comparator.naturalOrder()))));
    }
}
